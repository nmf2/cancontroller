...
typedef enum State {IDA, RTRA_SRR, IDE, r0, IDB, RTRB, r1_r0, DLC, PAYLOAD,
...
State last_state; // This var indicates the current sm's state.
Frame frame;

int bit_index;
int ifs_index; //Inter-frame space index
int idle_bus; // Bus-is-idle flag
int DLC_value; // Integer value of the DLC field.
int eol_recessive_count;
int eol_dominant_count;
int last_sample_point;
...
void CCL_sm(){
    ...
    if((btl_sample_point == 1) && (last_sample_point == 0)){
    ...
    last_sample_point = btl_sample_point;
...
void frame_walker(){
    ...
    if (state < INTERMISSION1){
        bit_index++;
        frame.data[bit_index] = Rx;
    }
