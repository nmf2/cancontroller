...
int btl_resize; // calculate the bit timing adjustment for resychronization due 
                // to any phase_error.
bool volatile btl_sample_point;  // indicate when the sample point happened.
bool volatile btl_writing_point; // indicate when the writing point happened.
        ...
        if(btl_hard_sync){
            ...
            if(btl_resync_enable){
                ...
                if(btl_current_state == BTL_STATE_TSEG1){
                    ...
                    // In this case, the phase error ir positive. The 
                    // adjustment due to a phase error is limited to SJW
                    btl_next_state = BTL_STATE_PESTATE1;
                    btl_resync_enable = false;
                }
                else if(btl_current_state == BTL_STATE_TSEG2){
                    btl_resize = min(BTL_SJW,-btl_phase_error);
                    // In this case, the phase error is negative. The 
                    // adjustment due to a phase error is limited to -SJW
...
void TOGGLE_write_serial(){// main.cpp
